...
import ForgetPassSDT from "./ForgetPassSDT";
...
function DoiMatKhau() {
  ...
  const Check_SDT = useSelector(checkForgetPassword_phone$);
  // lấy dữ liệu kiểm tra khi nhập otp chính xác
  const Check_OTP = useSelector(checkForgetPassword_otp$);
    ...
    >
      ...
      >
        ...
        {activeStep === steps.length ? (
          ...
          <React.Fragment>
            {activeStep === 0 ? <ForgetPassSDT /> : null}
            {activeStep === 1 ? <ForgetPassOTP /> : null}
            {activeStep === 2 ? <ChangePass /> : null}
